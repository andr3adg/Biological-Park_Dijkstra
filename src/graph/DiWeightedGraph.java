 ...
 * @param <E> Conection
 */
public class DiWeightedGraph<V, E> implements Graph<V, E> {
    ...
    @Override
    public Iterable<Edge<E, V>> ascendentEdges(Vertex<V> v) throws InvalidEdgeException {
        ...
        for (Edge<E, V> edge : edges.values()) {
            ...
            if( ((MyEdge)edge).vertices()[0].equals(v) ) { 
    ...
    public Iterable<Edge<E, V>> incidentEdges(Vertex<V> v) throws InvalidEdgeException {
        ...
        List<Edge<E,V>> ascendentEdges = new ArrayList<>();
        for (Edge<E, V> edge : edges.values()) {
            
            if( ((MyEdge)edge).vertices()[1].equals(v) ) { 
                ascendentEdges.add(edge);
            }
            
        }
    
        return ascendentEdges;
    }
    ...
    @Override
    public Edge<E, V> getEdge(Vertex<V> verticeA, Vertex<V> verticeB) {
        Edge<E, V> arestaRetorno = null;
        for (Edge<E, V> aresta : edges.values()) {
            if (aresta.vertices()[0] == verticeA && aresta.vertices()[1] == verticeB) {
                arestaRetorno = aresta;
            } else if (aresta.vertices()[1] == verticeA && aresta.vertices()[0] == verticeB) {
                arestaRetorno = aresta;
            }
        }
     ...
     */
    public Map<V, Vertex<V>> getVertexList(){
        return vertices;
    }
     ...
     */
    public Map<E, Edge<E, V>> getEdgesList(){
        return edges;
    }
    ...
    class MyEdge implements Edge<E, V> {
        ...
        public Vertex<V>[] vertices() {
            ...
            vertices [0] = vertexInbound;
            vertices [1] = vertexOutbound;
            return vertices;
        }
